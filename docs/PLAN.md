...
Replace/augment the Shell search provider: one box that mixes app launch, file hits (lexical+vector via Context Fabric), actions ("rotate this pdf" → tool from the bus), and calculator/unit answers from the system model with grammar-constrained output (no hallucinated math — the model routes to `qalc`, it doesn't do arithmetic). Latency budget: first useful results < 150 ms (lexical), semantic refinement streams in < 700 ms.
